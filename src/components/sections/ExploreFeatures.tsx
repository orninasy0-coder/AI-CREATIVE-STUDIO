  ...
  return (
    <section ref={ref} className="w-full bg-[#000] py-16 md:py-20">
      <div className="px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto">
        <motion.h2 className="font-[family-name:var(--font-space-grotesk)] text-2xl sm:text-3xl md:text-4xl font-bold text-white uppercase mb-8"
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag, i) => (
            <motion.span key={tag}
              className={`bg-white/[0.03] border border-white/[0.04] rounded-lg px-3 py-1.5 text-white/50 text-xs cursor-pointer transition-all duration-200 hover:bg-white/[0.06] hover:border-[#D7FF00]/20 hover:text-white/80 font-[family-name:var(--font-inter)] ${glowTags.has(tag) ? 'hover:shadow-[0_0_15px_rgba(215,255,0,0.06)]' : ''}`}
              initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
              transition={{ duration: 0.25, delay: i * 0.015 }} whileHover={{ scale: 1.03 }}>
              {tag}
            </motion.span>
          ))}
        </div>
