...
const presets = [
  ...
  { name: 'Cyberpunk', image: '/images/preset-4.png' },
  { name: 'Cinematic', image: '/images/preset-5.png' },
  { name: 'Anime Style', image: '/images/preset-6.png' },
  { name: 'Watercolor', image: '/images/preset-7.png' },
  { name: 'Vintage Film', image: '/images/preset-8.png' },
];
...
function PresetCard({ preset, index }: { preset: typeof presets[0]; index: number }) {
  return (
    ...
    >
      <div className="group bg-[#111] border border-white/[0.04] rounded-xl overflow-hidden cursor-pointer transition-all duration-300 hover:border-[#D7FF00]/20 hover:scale-[1.02]">
    ...
    <section ref={ref} className="w-full bg-[#000] py-20 md:py-28">
      <div className="px-5 sm:px-6 lg:px-8 max-w-[1440px] mx-auto">
        ...
        <motion.div className="relative" initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.1 }}>
          <button onClick={scrollPrev} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 z-10 w-10 h-10 bg-white/[0.04] hover:bg-white/[0.08] rounded-full flex items-center justify-center border border-white/[0.06] backdrop-blur-sm transition-colors" aria-label="Prev">
            <ChevronLeft className="w-4 h-4 text-white/60" />
          </button>
          <button onClick={scrollNext} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 z-10 w-10 h-10 bg-white/[0.04] hover:bg-white/[0.08] rounded-full flex items-center justify-center border border-white/[0.06] backdrop-blur-sm transition-colors" aria-label="Next">
